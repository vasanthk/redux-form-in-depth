...
export default function reduxFormLite(options) {
  const {
    form = '',
    fields: initFields = [],  // passed as default props to component.
    getInitialState,
    validate = () => {
    },
    handleSubmit = () => {
  ...
  // Add more decorations here if needed.
  ...
  return (Wrapped) => {
    class ReduxFormLite extends Component {
      ...
      setModel(newState) {
        ...
        this.values = simplifiedValues(fieldsKeys, newState); // Creates a key(field)-value object for all fieldKeys.
